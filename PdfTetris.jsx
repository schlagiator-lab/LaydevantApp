 ...
 * Prototype pour l'écran d'attente de la recherche web (Laydevant).
 *
 ...
 * la difficulté croît toute seule (ce qui manquait à l'invaders).
 *
 ...
 *
 * Le jeu vit SOUS le bandeau de recherche (la recherche reste la vedette).
...
const DEMO_QUERY = { brand: "ABB", model: "Welcome", type: "Interphone" };
...
function emptyGrid() {
  return Array.from({ length: CFG.rows }, () => Array(CFG.cols).fill(null));
}
...
export default function PdfTetris() {
  ...
  const collides = useCallback((shape, px, py) => {
    const st = g.current;
  ...
  const lockAndClear = useCallback(() => {
    ...
    st.softConsumed = true;   // soft drop verrouillé jusqu'au relâchement du doigt
    // le répit démarre APRÈS le flash s'il y en a un (sinon tout de suite)
    st.spawnLockUntil = performance.now() + (full.length ? 180 : 0) + CFG.spawnDelayMs;
  ...
  const applyClears = useCallback(() => {
    ...
    while (keep.length < CFG.rows) keep.unshift(Array(CFG.cols).fill(null));
    st.grid = keep;
    st.lines += cleared;
    setLines(st.lines);
  ...
  useEffect(() => {
    ...
    const drawCell = (gx, gy, color, ghost = false) => {
      const x = offX + gx * cell, y = offY + gy * cell;
      ctx.fillStyle = ghost ? "rgba(255,255,255,0.10)" : color;
    ...
    const drawExtLabel = (shape, px, py, ext, alpha = 1) => {
  ...
  // ---- gestes ----
  const touch = useRef({ x: 0, y: 0, lastColX: 0, moved: false, softOn: false });
  ...
  const onPointerMove = (e) => {
    ...
    // horizontal), au-delà d'un seuil franc, ET si le soft n'a pas été "consommé"
    // par un verrouillage récent (il faut relâcher le doigt pour le réarmer)
    if (!t.horiz && !g.current.softConsumed && dyTotal > CFG.softDropArm && Math.abs(dxTotal) < dyTotal && !t.softOn) {
  ...
  const onPointerUp = (e) => {
    ...
    g.current.softConsumed = false; // doigt relâché → soft drop réarmable
    const dyTotal = e.clientY - t.y;
    const dxTotal = e.clientX - t.x;
  ...
  useEffect(() => {
    ...
    const onKeyUp = (e) => { if (e.key === "ArrowDown") g.current.soft = false; };
  ...
  return (
    <div className="w-full min-h-screen flex items-start justify-center p-4" style={{ background: C.bgDeep }}>
      <div className="w-full max-w-sm">
        {/* ---- BANDEAU DE RECHERCHE (la vedette) ---- */}
        <div className="rounded-2xl px-4 py-3 mb-3" style={{ background: C.panel }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-semibold tracking-wide" style={{ color: C.textDim }}>RECHERCHE EN COURS…</span>
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full"
              style={{ background: "rgba(164,198,57,0.15)", color: C.green }}>● En ligne</span>
          </div>
          <div className="h-1.5 rounded-full overflow-hidden mb-2" style={{ background: "rgba(255,255,255,0.08)" }}>
            <div className="h-full rounded-full indet" style={{ background: C.orange, width: "34%" }} />
          </div>
          <p className="text-sm" style={{ color: C.text }}>
            <span style={{ color: C.textDim }}>On cherche&nbsp;:</span>{" "}
            <span style={{ color: C.green }}>{DEMO_QUERY.brand}</span>
            <span style={{ color: C.textDim }}> · </span>
            <span style={{ color: C.green }}>{DEMO_QUERY.model}</span>
            <span style={{ color: C.textDim }}> · </span>
            <span style={{ color: C.green }}>{DEMO_QUERY.type}</span>
          </p>
        </div>
        ...
        <div className="flex items-center justify-between mb-1 px-1">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="px-2 py-0.5 rounded-md font-mono font-semibold"
              style={{ background: "rgba(232,130,60,0.18)", color: C.orange }}>
              range un {target}
            </span>
          </div>
        ...
        <div ref={wrapRef}
          ...
          style={{ height: 460, border: `1px solid rgba(164,198,57,0.22)`, background: C.bg }}
          onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp}>
          <canvas ref={canvasRef} className="block" />
          {/* feedback "+bonus" quand une ligne cible est rangée */}
          {bonusFx > 0 && (
            <div key={bonusFx} className="absolute left-1/2 top-6 -translate-x-1/2 pointer-events-none bonusfx"
              style={{ color: C.shelf }}>
              <span className="text-lg font-bold">+{CFG.bonusPoints} bon dossier ! ⚡</span>
            </div>
          )}
          {over && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3"
              style={{ background: "rgba(15,36,68,0.86)" }}>
              <p className="text-lg font-semibold" style={{ color: C.text }}>Bibliothèque pleine !</p>
              <p className="text-sm" style={{ color: C.textDim }}>
                score {score} · {lines} documents rangés
              </p>
              <button onClick={restart}
                className="px-4 py-2 rounded-xl text-sm font-semibold"
                style={{ background: C.green, color: "#173000" }}>
                Rejouer
              </button>
            </div>
          )}
        ...
        <p className="text-center mt-2 text-xs" style={{ color: "#5a6f92" }}>
          Glisse ← → pour ranger · tap pour tourner · glisse ↓ pour accélérer · prototype de réglage
        </p>
      </div>

      <style>{`
        @keyframes indet { 0% { transform: translateX(-120%);} 100% { transform: translateX(320%);} }
        .indet { animation: indet 1.6s ease-in-out infinite; }
        @keyframes bonusfx {
